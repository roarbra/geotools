...
public class OSMService extends WebMercatorTileService {
    ...
    public Tile findTileAtCoordinate(double lon, double lat, ZoomLevel zoomLevel) {
        lat = TileService.normalizeDegreeValue(lat, 90);
        lon = TileService.normalizeDegreeValue(lon, 180);
        ...
        if (yTile < 0) yTile = 0;
        OSMTileIdentifier id = new OSMTileIdentifier(xTile, yTile, zoomLevel, getName());
